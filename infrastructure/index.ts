...
const cache = new redis.Redis(`${prefixName}-redis`, {
...
const redisAccessKey = redis
  .listRedisKeysOutput({ name: cache.name, resourceGroupName: resourceGroup.name })
...
const redisConnectionString = pulumi.interpolate`rediss://:${redisAccessKey}@${cache.hostName}:${cache.sslPort}`
